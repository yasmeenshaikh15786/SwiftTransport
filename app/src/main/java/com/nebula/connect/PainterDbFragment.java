 ...
 */
public class PainterDbFragment  extends Fragment {

    private static final String TAG  = PainterDbFragment.class.getSimpleName();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_painterdb, container, false);

        adapter = new CustomList();

        saleMetadataEntities = SelectQueries.getSaleMetadataElementsByTag(getActivity(),getActivity().getIntent().getIntExtra("plan_id",0),Constants.PHOTO_PAINTER);
        ...
        try {
            if(resultCode == Activity.RESULT_OK && requestCode == PHOTO_SAVED) {
                ...
                if(currentEntityPosition == -1) {
                    ...
                    entity.tag = Constants.PHOTO_PAINTER;
                    ...
                    saleMetadataEntities.get(currentEntityPosition).tag = Constants.PHOTO_PAINTER;
                }
    ...
    public void addShop(View v){
        ...
        if(v.getId()==R.id.add){
            String maxPhotoLimit = SelectQueries.getSetting(getContext(),Settings.MAX_PAINTER_COUNT);
